...
git fetch git@github.com:llvm-mirror/llvm.git release_50 --depth=1
...
git archive --format=tar.gz FETCH_HEAD --prefix=llvm-$GIT_HASH/ > $GIT_HASH.tar.gz
...
cat ${BASE}.spec \
   ...
   | sed -E "s#^[%]setup -q -n llvm-.*\$#%setup -q -n llvm-${GIT_HASH}#g"  \
 > _temp.spec
